...
fn global_search(cx: &mut Context) {
    ...
    let initial_query = cx
        .register
        .and_then(|reg| cx.editor.registers.last(reg))
        .cloned()
        .unwrap_or_default();
    let files_query = get_files(initial_query.clone(), cx.editor);

    let show_picker = async move {
        let query_result = files_query.await;

        let call: job::Callback = Callback::EditorCompositor(Box::new(
            move |editor: &mut Editor, compositor: &mut Compositor| {
                let all_matches = match query_result {
                    Ok(matches) => matches,
                    Err(err) => {
                        editor.set_error(format!("Failed to run search: {}", err));
                        return;
                    }
                };
...
fn jumplist_picker(cx: &mut Context) {
    ...
    let columns = vec![
        ui::PickerColumn::new("id", |item: &JumpMeta| item.id.to_string().into()),
        ui::PickerColumn::new("path", |item: &JumpMeta| {
            let path = item
                .path
                .as_deref()
                .map(helix_core::path::get_relative_path);
        ...
        ui::PickerColumn::new("flags", |item: &JumpMeta| {
        ...
        ui::PickerColumn::new("contents", |item: &JumpMeta| item.text.as_str().into()),
    ];
